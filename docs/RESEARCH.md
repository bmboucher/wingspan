...
- **Setup enumeration + scoring.** `setup_model.enumerate_setup_candidates(dealt_cards, dealt_bonus)` already produces the exact **504-option** keep set (same order the engine offers) *under the function's defaults*, and `SetupNet` + `setup_model.candidates.select_by_margins` score and pick one. Note: present-day default runs (`TrainConfig.split_setup_bonus=True`, `split_setup_food=True`) defer both the bonus and food picks to in-game heads, so the setup keep itself only enumerates **32 candidates** (card subsets only) — the full 504-option space applies to the un-split, combined-keep regime. Project 4's "generate the options for a hand and score them" is essentially already implemented either way — it just needs to be driven over a large sample and tallied.
...
- **Generic event reporting framework.** `wingspan.instrumentation` exposes 16 typed game events — the full game/round/turn lifecycle (`GAME_START/END`, `ROUND_START/END`, `TURN_START/END`), the decision flow (`MAKING_DECISION`, `MADE_DECISION`), and domain-specific events (`BIRD_PLACED`, `FOOD_GAINED`, `EGGS_LAID`, `CARDS_DRAWN`, `ROUND_GOAL_SCORED`, `PLAYER_FINAL_SCORED`, `SETUP_APPLIED`, `SETUP_START`) — via a `Instrumentation` dispatcher that the engine holds. Handlers are Pydantic models (`CallbackHandler` subclasses) registered by name with `@registry.register`; an `InstrumentationConfig` wires named handler instances to their events, serializes/deserializes through `model_dump`/YAML, and round-trips through checkpoints. Three bundled handlers ship as reference implementations: `DecisionLogger` (one JSONL row per genuine decision), `CardVisitRecorder` (per-game bird-play tally), and `GameLogHtmlHandler` (per-phase state snapshots rendered into a navigable self-contained HTML log viewer per game). An engine with no instrumentation holds the shared empty router — no per-call overhead.
...
- **Artifact versioning (MODEL_VERSION 1.5) + compat shim chain.** The `wingspan.compat` package now carries the `v1_0`, `v1_3`, and `v1_4` era shims (the pre-1.0 shims, v0.0 through v0.7, were dropped at the 1.0 MAJOR bump), the corresponding era fixture sets are committed, and the FRESH/REGIME versioning policy is enforced in code (inference loaders check the artifact version and refuse mismatches cleanly).
...
- [x] **Per-block importance probe.** `wingspan analysis probe` (`wingspan.analysis`) measures a *given* trained checkpoint's board-attention behavior/ablation sensitivity and trunk/choice-encoder layer capacity — not a sweep answer to "which preset wins" (the items below are still open), but a direct read of what a net's blocks are actually doing. *2026-09-15 findings: the board-attention block behaves mostly as a mean-pool summary (six of eight heads near-uniform entropy), confirmed by a 49.6% ± 8.9 full-vs-uniform-substitution head-to-head win rate; the trunk and choice encoder's tail layers are sharply low-rank (7/64 and 3/64 effective dims, linear R² ≈ 0.99) — capacity to reclaim by narrowing, not a case for widening.*
...
Scoring the options for a hand is already implemented (`enumerate_setup_candidates` + `SetupNet`/`select_by_margins`) — 504 options in the combined-keep regime, or 32 card-only options under today's default split-bonus/split-food config (see the gap note above). The missing pieces are the sampler and the aggregation, both embarrassingly parallel.

- [x] **Mass-sampling harness.** Sample a few million deals (5 birds + 2 bonus per seat from the deck), enumerate + score, take the policy's pick (argmax), and tally per-card "selected-when-present" frequency — Q1, and the "ideal hand" (Q3). Built entirely on existing primitives. *Done 2026-09-15 as `wingspan research setup-keep` (`wingspan.research.setup_keep`): deals seeded games through the engine's own deal path, scores every candidate keep of every seat through the run's `SetupNet` in batched forward passes (in-process or a `--workers` process pool), and writes an actuarial **experience table** — one exposure row per dealt bird with a 0/1 `kept` observation, the policy's marginal `keep_prob`, the critic's `deal_value`, the card's printed attributes, and the rest of the setup (tray, feeder, goals, bonus offer) — so keep rate = `sum(kept) / count` pivots by any column. Q3 (the ideal hand) is a pivot over the same table; the tally itself is left to the analysis notebook.*
