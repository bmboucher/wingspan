...
3. ✅ **PPO + GAE shipped** (§3.4). The algorithm upgrade is done: PPO's clipped
   ...
   REINFORCE behavior exactly. ✅ **Gradient-accumulation minibatching also shipped**
   (§3.3/§4.2b): the `update_minibatch_steps` REGIME knob (default `0` = whole-batch,
   today's exact behavior) splits the flattened batch into minibatches and accumulates
   gradients before each `optimizer.step()`, capping peak RAM at the minibatch size
   regardless of `games_per_iter`. Recommended ≈ 4096 for large-batch runs.
...
- **Self-play.** Every seat is driven by the *same* network (2 by default; a
  run can train at up to 5). Symmetry comes from encoding the board from the
  deciding player's point of view (DECISIONS.md §0) and giving every seat a
  reward relative to the field it actually faced.
- **Reward / return (G).** What we want to maximize. Two orthogonal knobs:
  ...
  - `reward_basis` controls *what value* is used. Default (`margin`): a seat's
    own score minus the *best* other seat's score — margin against the
    strongest opponent it actually faced, not an average. Formally, for seat
    `i` with scores `s`: `value_i = s[i] − max(s[j] for j != i)`. An
    `end_game_bonus` (if configured) is added on top: `+bonus` to the sole
    winner and `−bonus` to **every** other seat when the game has one
    (`winner >= 0`); `0` for everyone on a shared victory (`winner == -1`, no
    sole winner — see `scoring.winners`/`determine_winner`). At 2 players
    "best other" collapses to the single opponent, so this is *exactly* the
    legacy `(s0 − s1 + b0, s1 − s0 − b0)` formula — same seats, same signs,
    same bytes (`wingspan.training.returns.terminal_values`). Opt-in
    (`own_score`): each player's own absolute final score — every seat
    receives a non-negative reward, gradient pushes toward *scoring more*
    regardless of the opponents; `end_game_bonus` is added only for the sole
    winner. The two axes are fully independent (four combinations). All are
    shape-preserving (REGIME — no checkpoint restart).
...
`VALUE_COEF = 0.5`, `ENTROPY_COEF = 0.01` (anneal toward a smaller value late in
training so the policy is allowed to sharpen — see "Annealing schedules" below),
gradient clipping at `5.0` (already present — caps the update size so one freak
batch cannot wreck the weights).
...
**How often to update.** ✅ **Gradient-accumulation minibatching shipped.**

> **Term — minibatch:** rather than computing the gradient over all ~18 k steps
> at once (memory-heavy), split the batch into chunks of a few thousand steps,
> accumulate gradients across them, and take one optimizer step per epoch — same
> gradient, lower peak memory.

Set `training.update_minibatch_steps` (REGIME knob, default `0` = whole batch).
Recommended: **≈ 4,096 steps**. With a 1000-game iteration (~187 k steps)
this splits the backward into ~45 passes that free their activation graphs
individually, keeping peak RAM proportional to the minibatch size rather than
the full batch. The implementation uses **gradient accumulation** (one
`optimizer.step()` per epoch), not per-minibatch stepping — so the number of
optimizer steps per iteration stays the same as today (1 for REINFORCE, 4 for
PPO with `ppo_reuse_epochs=4`). Per-minibatch SGD (more steps per iteration,
~15 updates) is a potential future upgrade; today's choice avoids changing
learning dynamics. The DAgger clone phase (§6.8) is the one exception: it
steps once per minibatch rather than once per epoch, because supervised
cloning has no on-policy constraint to preserve (§6.8 "Why cloning steps per
minibatch").
...
**Annealing schedules.** ✅ **Shipped** — the entropy bonus and dropout
probability can each taper linearly from an initial value to a final value,
reaching the final value at `run.target_iterations` and holding constant
beyond it. Set `training.entropy_coef_final` / `training.dropout_final` (main
net) and/or `training.setup.entropy_coef_final` / `training.setup.dropout_final`
(setup net); `None` (the default) means no anneal — the value stays constant at
its initial. A typical run tapering both entropy and dropout from `0.05` down
to `0.01`:

```
training.entropy_coef = 0.05,        training.entropy_coef_final = 0.01
architecture.main.dropout = 0.05,    training.dropout_final = 0.01
...
These are REGIME (training-only) fields: they never enter `ModelArchitecture` /
`architecture_key`, so they carry no `MODEL_VERSION` bump and are invisible to
inference rehydration (`runmeta.ModelConfig`) — only the training loop reads
them, via `RunConfig.entropy_coef_at` / `dropout_p_at` (and their `setup_*`
mirrors) keyed to the absolute iteration counter (`docs/VERSIONING.md`).
...
2. ✅ **PPO (Proximal Policy Optimization)** — **shipped**. Set via
   ...
   - **Note:** currently full-batch per epoch (no per-minibatch shuffling between
     epochs). ✅ **Gradient-accumulation minibatching** (§4.2b) is now shipped as
     `training.update_minibatch_steps` (default `0` = whole-batch). It reduces
     peak memory by processing the batch in sequential chunks, accumulating
     gradients, and calling `optimizer.step()` once per epoch — same as today's
     gradient and step count. Per-epoch shuffling between chunks is not done (a
     future upgrade if needed); the accumulation is already effective as a memory
     cap.
...
4. The learner updates via **cross-entropy to the expert's soft targets** (plus
   ...
   terms are disabled. Unlike the RL update (§3.3: one accumulated optimizer
   step per epoch), the clone-phase update takes **one optimizer step per
   minibatch**: each iteration's collected batch is shuffled and split into
   `dagger.clone_minibatch_steps`-sized minibatches, and the learner steps once
   per minibatch, repeated for `dagger.clone_epochs` freshly-shuffled epochs.
   The loss for each minibatch is:
...
#### Why cloning steps per minibatch

The RL update (§3.3) takes one accumulated gradient step per epoch — right for
on-policy data, where the whole batch reflects a single behavior policy and
averaging the gradient over it keeps the update on-policy. Supervised cloning
has no such constraint: the targets are fixed (the frozen expert's soft
distribution), so there is no reason to average a whole iteration's gradient
into one step.

The first real clone run (`runs/arch_pair`, 2026-09-24, 25 clone iterations at
1000 games/iteration) went through the RL accumulated-step path — one
optimizer step per iteration. The imitation cross-entropy stayed essentially
flat across the whole clone phase (1.167 → 1.163) and the policy entered PPO
training at random-init entropy (the clone phase had not moved it). The only
signal that did take hold in those 25 steps was the value-MSE term, whose
target against a fixed strong opponent is nearly constant, and it collapsed the
un-normalised trunk to rank 1 (the in-loop probes show trunk rank95 falling
70 → 1 by iteration 20). Stepping once per minibatch instead of once per
epoch gives the loss `clone_epochs * ceil(batch_steps / clone_minibatch_steps)`
gradient steps per clone iteration (`batch_steps` = the iteration's flattened
decision count; ≈180 steps at 1000 games/iteration with the defaults) instead
of one, which is what makes the cross-entropy move.

Two things to keep in mind when reading `imitation_loss`. It is a
cross-entropy, so its floor is the expert's own entropy on the student's
states, not zero: a flat curve near that floor means the student already
matches the expert, while a flat curve well above it means it is not learning.
Measured on a fresh student's states against the arch_pair champion, the
expert's entropy is ≈0.21 nats and the uniform-policy level (mean `ln k` over
legal options) is ≈1.17 — so the run's flat 1.167 was exactly the uniform
level. The same 24-game smoke with the per-minibatch schedule
(`clone_minibatch_steps` 256) took the loss 1.12 → 0.62 in six iterations and
the margin against the champion from −59 to about −35; the single-step path
stayed at 1.16–1.18 and −55 to −60 throughout.
And each clone iteration's update now costs roughly `clone_epochs` forward and
backward passes over the batch instead of one, so expect the UPDATE line to
take about four times as long during the clone phase.
...
train:
  ...
  dagger:
    ...
    clone_epochs: 4             # shuffled passes over the batch per clone iteration
    clone_minibatch_steps: 2048 # one optimizer step per this many flattened steps
```
...
> **Status.** `evaluate.evaluate_vs_opponent` plays `n_pairs` deals against a
> fixed reference opponent in greedy mode, rotating the net through *every*
> seat of each deal (`n_games = num_players * n_pairs`; 2 seats — the legacy
> mirrored pair — by default), and returns an `EvalResult` with win rate, a
> 95 % CI, mean margin, and the opponent's generation. Each game's margin is
> the net's score minus the *best* other seat's score; each game's win credit
> (`metrics.EvalGameOutcome.win_credit`, averaged into `win_rate`) is `1.0`
> when the net's seat is the sole winner, `1/k` on a `k`-way shared victory
> that includes it, else `0.0` (`scoring.winners`) — at 2 players this is the
> legacy "tie counts as half a win" rule, except a score tie can now resolve
> to a sole winner via the official food-supply tiebreak, so `margin == 0` no
> longer implies `0.5` credit. The reference is the **random agent** during
> the bootstrap phase, then an **advancing frozen self** (`opponent.pt`) once
> the policy graduates (`loop_eval`); a fresh challenger — and a bootstrap
> collection win-rate — starts even at the *neutral* baseline `100/num_players`
> percent (50 % at 2 players), not a hardcoded 50 %. ☐ Still open: a fixed
> heuristic agent, a *multi-checkpoint* league (only the single advancing
> opponent is kept today), and Elo over it.
...
evaluation deal `num_players` times — once with your policy in each seat, on
the **same seed** — and average (2 rotations, i.e. the classic mirrored pair,
at the default seat count). This cancels the deal/first-player luck and gives
a far tighter estimate from the same number of games.
