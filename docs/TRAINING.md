...
- **Reward / return (G).** What we want to maximize. Default (`reward_mode =
  terminal_margin`): the **final score margin** (your score minus the
  opponent's), the same for every step you took in that game. Opt-in
  alternative (`reward_mode = decision_delta`): each decision's return is the
  `reward_discount` (γ)-discounted sum of the **per-decision margin changes**
  from that step onward, so a decision is credited with the points it (and the
  interval until your next decision) actually moved. γ=0 keeps only the
  immediate change; γ=1 telescopes to the final margin minus the margin before
  that decision. Both modes give the two seats opposite signs and are
