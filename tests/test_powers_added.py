...
def test_full_coverage_no_unimplemented():
    ...
    bad = [
        b.name
        for b in birds
        if any(e.kind == cards.EffectKind.UNIMPLEMENTED for e in b.power.effects)
    ]
...
def test_parser_tuck_from_deck_paid():
    ...
    eff = next(e for e in p.effects if e.kind == cards.EffectKind.TUCK_FROM_DECK_PAID)
...
def test_parser_predator_hunt():
    ...
    eff = next(e for e in p.effects if e.kind == cards.EffectKind.PREDATOR_HUNT)
...
def test_parser_move_rightmost():
    ...
    assert any(e.kind == cards.EffectKind.MOVE_BIRD_IF_RIGHTMOST for e in p.effects)
...
def test_parser_repeat_brown():
    ...
    assert any(e.kind == cards.EffectKind.REPEAT_BROWN_POWER for e in p.effects)
...
def test_parser_pink_lay_egg_on_nest():
    ...
    eff = next(e for e in p.effects if e.kind == cards.EffectKind.PINK_LAY_EGG_ON_NEST)
...
def test_parser_pink_predator_feeder():
    ...
    assert any(e.kind == cards.EffectKind.PINK_PREDATOR_FEEDER for e in p.effects)
...
def test_draw_bonus_keep_keeps_one_discards_one():
    ...
    p.board[cards.Habitat.WETLAND].append(pb)
...
def test_predator_hunt_tucks_small_bird():
    ...
    p.board[cards.Habitat.FOREST].append(pb)
    # Force a small bird on top of the deck.
    small = next(
        b for b in birds if b.wingspan_cm and b.wingspan_cm < 30 and b is not hawk
    )
...
def test_move_rightmost_moves_when_rightmost():
    ...
    other = next(
        b for b in birds if b.name != mover.name and cards.Habitat.FOREST in b.habitats
    )
    p = eng.state.me()
...
def test_repeat_brown_replays_neighbor_power():
    ...
    p.board[cards.Habitat.FOREST].extend([pb_target, pb_cat])
...
def test_pink_lay_eggs_reactor_fires_on_opponent_lay_eggs():
    ...
    bowl_bird = next(
        b
        for b in birds
        if b.nest == cards.NestType.BOWL and b.egg_limit > 0 and b.name != cowbird.name
    )
    ...
    actions.do_lay_eggs(eng, lambda _e, d: d.choices[0])
