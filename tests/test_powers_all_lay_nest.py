...
from wingspan.cards import EffectKind, NestType, load_all
...
def test_all_players_lay_on_nest_parsed():
    ...
    for name, expected_nest in cases:
        ...
        effs = [
            ...
            if e.kind == EffectKind.ALL_PLAYERS_LAY_EGG_ON_NEST
        ]
        assert effs, f"{name} missing ALL_PLAYERS_LAY_EGG_ON_NEST effect"
        assert (
            effs[0].nest == expected_nest
        ), f"{name}: expected {expected_nest}, got {effs[0].nest}"
