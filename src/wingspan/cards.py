...
class EffectKind(str, Enum):
    ...
    GAIN_FOOD_BIRDFEEDER_CHOICE = "gain_food_birdfeeder_choice"  # extra=(Food.A, Food.B)
    GAIN_BIRDFEEDER_DIE = "gain_birdfeeder_die"  # amount=number of dice
    ...
    DRAW_BONUS_KEEP = "draw_bonus_keep"  # amount=draw_count, extra=(keep_count,)
    PINK_OPP_PREDATOR_DIE = "pink_opp_predator_die"
    TRADE_FOOD = "trade_food"
    DRAW_TRAY_ALL = "draw_tray_all"
    EACH_PLAYER_GAIN_BIRDFEEDER_DIE = "each_player_gain_birdfeeder_die"
    GAIN_ALL_FOOD_BIRDFEEDER = "gain_all_food_birdfeeder"  # food=food_type
    DISCARD_EGG_GAIN_WILD = "discard_egg_gain_wild"  # amount=food_count_to_gain
    DISCARD_FOOD_TUCK_FROM_DECK = "discard_food_tuck_from_deck"  # food=food_type, amount=tuck_count
    PREDATOR_TUCK = "predator_tuck"  # extra=(max_wingspan_cm,)
    LAY_EGG_ALL_NEST = "lay_egg_all_nest"  # extra=(NestType.X,)
    ALL_PLAYERS_LAY_ON_NEST = "all_players_lay_on_nest"  # extra=(NestType.X,)
    LEAST_BIRDS_GAIN_DIE = "least_birds_gain_die"  # habitat=Habitat.X
    REPEAT_BROWN = "repeat_brown"
    REPEAT_PREDATOR = "repeat_predator"
...
def parse_power(color: PowerColor, text: str) -> Power:
    ...
    # Pattern: "Gain 1 [foodA] or [foodB] from the birdfeeder, if available."
    m = re.search(r"Gain\s+1\s+(\[\w+\])\s+or\s+(\[\w+\])\s+from the birdfeeder", t, re.I)
    if m and m.group(1) in FOOD_TAGS and m.group(2) in FOOD_TAGS:
        food_a = FOOD_TAGS[m.group(1)]
        food_b = FOOD_TAGS[m.group(2)]
        effects.append(Effect(EffectKind.GAIN_FOOD_BIRDFEEDER_CHOICE, extra=(food_a, food_b), raw_text=m.group(0)))

    # Pattern 1b: "Gain N [die] from the birdfeeder." — [die] = player's choice of food
    m = re.search(r"Gain\s+(\d+|a|an|one|two|three)\s+\[die\]\s+from the birdfeeder", t, re.I)
    if m:
        n = _to_int(m.group(1)) or 1
        effects.append(Effect(EffectKind.GAIN_BIRDFEEDER_DIE, amount=n, raw_text=m.group(0)))
    ...
    # Pattern: "Gain all [food] that are in the birdfeeder."
    m = re.search(r"Gain all\s+(\[\w+\])\s+that are in the birdfeeder", t, re.I)
    if m and m.group(1) in FOOD_TAGS:
        effects.append(Effect(EffectKind.GAIN_ALL_FOOD_BIRDFEEDER, food=FOOD_TAGS[m.group(1)], raw_text=m.group(0)))
    ...
    # Pattern 7: "Play an additional bird in your [habitat]" or "in this bird's habitat"
    ...
    elif re.search(r"Play an additional bird in this bird's habitat", t, re.I):
        # habitat=None means "same habitat as this bird" — the engine already handles None
        effects.append(Effect(EffectKind.PLAY_ADDITIONAL_BIRD, habitat=None, raw_text=t))
    ...
    # Pattern: "Draw N new bonus cards and keep K."
    m = re.search(r"Draw\s+(\d+|a|an|one|two|three)\s+new bonus cards? and keep\s+(\d+|a|an|one)", t, re.I)
    if m:
        draw_n = _to_int(m.group(1)) or 2
        keep_n = _to_int(m.group(2)) or 1
        effects.append(Effect(EffectKind.DRAW_BONUS_KEEP, amount=draw_n, extra=(keep_n,), raw_text=m.group(0)))
    ...
    # Pattern: "Look at a [card] from the deck. If less than Xcm, tuck it behind this bird. If not, discard it."
    m = re.search(r"Look at a \[card\] from the deck\. If less than (\d+)cm", t, re.I)
    if m:
        threshold = int(m.group(1))
        effects.append(Effect(EffectKind.PREDATOR_TUCK, extra=(threshold,), raw_text=m.group(0)))

    # Pattern: "Discard 1 [food] to tuck N [card] from the deck behind this bird."
    m = re.search(r"Discard\s+1\s+(\[\w+\])\s+to tuck\s+(\d+|a|an|one|two|three)\s+\[card\] from the deck behind this bird", t, re.I)
    if m and m.group(1) in FOOD_TAGS:
        n = _to_int(m.group(2)) or 2
        effects.append(Effect(EffectKind.DISCARD_FOOD_TUCK_FROM_DECK, amount=n, food=FOOD_TAGS[m.group(1)], raw_text=m.group(0)))

    # Pattern: "Discard 1 [egg] from any of your other birds to gain N [wild] from the supply."
    m = re.search(r"Discard\s+1\s+\[egg\] from any of your other birds to gain\s+(\d+|a|an|one|two)\s+\[wild\] from the supply", t, re.I)
    if m:
        n = _to_int(m.group(1)) or 1
        effects.append(Effect(EffectKind.DISCARD_EGG_GAIN_WILD, amount=n, raw_text=m.group(0)))

    # Pattern: "Lay 1 [egg] on each of your birds with a [nest] nest."
    m = re.search(r"Lay\s+1\s+\[egg\] on each of your birds with a (\[\w+\]) nest", t, re.I)
    if m and m.group(1) in NEST_TAGS:
        nest = NEST_TAGS[m.group(1)]
        effects.append(Effect(EffectKind.LAY_EGG_ALL_NEST, extra=(nest,), raw_text=m.group(0)))

    # Pattern: "Each player gains 1 [die] from the birdfeeder, starting with the player of your choice."
    if re.search(r"Each player gains\s+1\s+\[die\] from the birdfeeder", t, re.I):
        effects.append(Effect(EffectKind.EACH_PLAYER_GAIN_BIRDFEEDER_DIE, raw_text=t))

    # Pattern: "Draw the N face-up [card] in the bird tray."
    if re.search(r"Draw the\s+\d+\s+face-up\s+\[card\]", t, re.I):
        effects.append(Effect(EffectKind.DRAW_TRAY_ALL, raw_text=t))

    # Pattern: "All players lay 1 [egg] on any 1 [nest] bird. You may lay 1 [egg] on 1 additional [nest] bird."
    m = re.search(r"All players lay\s+1\s+\[egg\] on any\s+1\s+(\[\w+\]) bird", t, re.I)
    if m and m.group(1) in NEST_TAGS:
        nest = NEST_TAGS[m.group(1)]
        effects.append(Effect(EffectKind.ALL_PLAYERS_LAY_ON_NEST, extra=(nest,), raw_text=m.group(0)))

    # Pattern: "Trade 1 [wild] for any other type from the supply."
    if re.search(r"Trade\s+1\s+\[wild\] for any other type from the supply", t, re.I):
        effects.append(Effect(EffectKind.TRADE_FOOD, raw_text=t))

    # Pattern: "When another player's [predator] succeeds, gain 1 [die] from the birdfeeder."
    if re.search(r"When another player's\s+\[predator\]\s+succeeds,\s+gain\s+1\s+\[die\] from the birdfeeder", t, re.I):
        effects.append(Effect(EffectKind.PINK_OPP_PREDATOR_DIE, raw_text=t))

    # Pattern: "Draw [card] equal to the number of players +1. Starting with you..."
    if re.search(r"Draw\s+\[card\]\s+equal to the number of players", t, re.I):
        effects.append(Effect(EffectKind.DRAFT_CARDS, raw_text=t))

    # Pattern: "Player(s) with the fewest birds in their [habitat] gain 1 [die] from birdfeeder."
    m = re.search(r"Player\(s\) with the fewest birds in their (\[\w+\]) gain\s+1\s+\[die\] from birdfeeder", t, re.I)
    if m and m.group(1) in HABITAT_TAGS:
        effects.append(Effect(EffectKind.LEAST_BIRDS_GAIN_DIE, habitat=HABITAT_TAGS[m.group(1)], raw_text=m.group(0)))

    # Pattern: "Repeat 1 [predator] power in this habitat."
    if re.search(r"Repeat\s+1\s+\[predator\] power in this habitat", t, re.I):
        effects.append(Effect(EffectKind.REPEAT_PREDATOR, raw_text=t))

    # Pattern: "Repeat a brown power on another bird in this habitat."
    if re.search(r"Repeat a brown power on another bird in this habitat", t, re.I):
        effects.append(Effect(EffectKind.REPEAT_BROWN, raw_text=t))
