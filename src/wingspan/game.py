...
class Engine:
    ...
    def _setup_phase(self, agents):
        ...
        for p in st.players:
            ...
            self._draw_and_keep_bonus(
                agents[p.id], p,
                draw_n=STARTING_BONUS_CARDS_DEAL, keep_n=1,
                prompt=f"[{p.name}] choose a bonus card to keep",
            )
            if p.bonus_cards:
                self._log(f"[{p.name}] keeps bonus '{p.bonus_cards[-1].name}'")
    ...
    def _lay_one_egg(self, agent: Agent, p: Player, nest_type: Optional[NestType] = None, optional: bool = False) -> bool:
        choices = []
        for h, row in p.board.items():
            for i, pb in enumerate(row):
                if pb.eggs < pb.bird.egg_limit and (nest_type is None or pb.bird.nest == nest_type):
        ...
        prompt = f"[{p.name}] lay 1 egg on a {nest_type.value} bird" if nest_type else f"[{p.name}] lay 1 egg"
        ch = self._ask(agent, Decision(
    ...
    def _lay_one_egg_on_nest(self, agent: Agent, p: Player, nest_type: NestType, optional: bool = False) -> bool:
        return self._lay_one_egg(agent, p, nest_type=nest_type, optional=optional)
    ...
    # Birdfeeder helpers
    # ------------------------------------------------------------------
    def _gain_one_birdfeeder_die(self, agent: Agent, p: Player, bird_name: str) -> bool:
        """Ask *p* to take one die from the birdfeeder; reroll on empty and after the pick if only one face remains.

        Returns True when a die was taken, False when the birdfeeder is completely exhausted.
        """
        st = self.state
        ...
        if not avail:
            if st.birdfeeder.total() > 0:
                ...
                self._log("  birdfeeder empty; rerolled")
                avail = [(f, c) for f, c in st.birdfeeder.counts.items() if c > 0]
            if not avail:
                return False
        ch = self._ask(agent, Decision(
            type=DecisionType.GAIN_FOOD_PICK_DIE,
            player_id=p.id,
            prompt=f"[{p.name}] gain 1 die from birdfeeder ({bird_name})",
            choices=[Choice(label=f"{f.value}({c})", payload=f) for f, c in avail],
        ))
        chosen: Food = ch.payload
        st.birdfeeder.counts[chosen] -= 1
        p.food[chosen] += 1
        self._log(f"  {bird_name}: +1 {chosen.value} from birdfeeder")
        if sum(1 for c in st.birdfeeder.counts.values() if c > 0) <= 1 and st.birdfeeder.total() > 0:
            st.birdfeeder.reroll(st.rng)
        return True
    ...
    # Bonus-card helpers
    # ------------------------------------------------------------------
    def _draw_and_keep_bonus(self, agent: Agent, p: Player, draw_n: int, keep_n: int, prompt: str) -> None:
        """Draw ``draw_n`` bonus cards, let the player keep ``keep_n``, discard the rest."""
        st = self.state
        drawn = [st.bonus_deck.pop() for _ in range(draw_n) if st.bonus_deck]
        if not drawn:
            return
        for _ in range(min(keep_n, len(drawn))):
            if len(drawn) == 1:
                p.bonus_cards.append(drawn.pop())
            else:
                ch = self._ask(agent, Decision(
                ...
                chosen = ch.payload
                p.bonus_cards.append(chosen)
                drawn.remove(chosen)
        st.bonus_discard.extend(drawn)
    ...
    def _activate_row_powers(self, agent: Agent, p: Player, habitat: Habitat):
        ...
        for pb in reversed(p.board[habitat]):
            ...
            if pb.bird.predator:
                self._trigger_pink_predator(agent, p)

    def _trigger_pink_predator(self, agent: Agent, active_player: Player) -> None:
        """Fire PINK_OPP_PREDATOR_DIE effects on opponents when active player's predator activates."""
        for q in self.state.players:
            if q.id == active_player.id:
                continue
            for h, row in q.board.items():
                for pb in row:
                    ...
                    for eff in pb.bird.power.effects:
                        if eff.kind == EffectKind.PINK_OPP_PREDATOR_DIE:
                            self._apply_effect(agent, q, pb, h, eff, "pink")
    ...
    def _apply_effect(self, agent: Agent, p: Player, pb: PlayedBird, habitat: Habitat, eff: Effect, trigger: str):
        ...
        if eff.kind == EffectKind.GAIN_FOOD_SUPPLY:
        ...
        elif eff.kind == EffectKind.GAIN_FOOD_BIRDFEEDER_CHOICE:
            options = [f for f in eff.extra if isinstance(f, Food) and st.birdfeeder.counts.get(f, 0) > 0]
            if not options:
                self._log(f"  {bird.name}: no [{'/'.join(f.value for f in eff.extra)}] in birdfeeder, skipped")
            else:
                if len(options) == 1:
                    chosen = options[0]
                else:
                    ch = self._ask(agent, Decision(
                        type=DecisionType.GAIN_FOOD_PICK_DIE,
                        player_id=p.id,
                        prompt=f"[{p.name}] gain 1 food from birdfeeder ({bird.name})",
                        choices=[Choice(label=f.value, payload=f) for f in options],
                    ))
                    chosen = ch.payload
                st.birdfeeder.counts[chosen] -= 1
                p.food[chosen] += 1
                self._log(f"  {bird.name}: +1 {chosen.value} from birdfeeder")
        elif eff.kind == EffectKind.GAIN_ALL_FOOD_BIRDFEEDER:
            if eff.food:
                n = st.birdfeeder.counts.get(eff.food, 0)
                if n > 0:
                    st.birdfeeder.counts[eff.food] = 0
                    p.food[eff.food] += n
                    self._log(f"  {bird.name}: +{n} {eff.food.value} from birdfeeder (all)")
                else:
                    self._log(f"  {bird.name}: no {eff.food.value} in birdfeeder")
        elif eff.kind == EffectKind.GAIN_BIRDFEEDER_DIE:
            for _ in range(eff.amount):
                if not self._gain_one_birdfeeder_die(agent, p, bird.name):
                    break
            ...
            keep_n = eff.extra[0] if eff.extra else 1
            self._draw_and_keep_bonus(
                agent, p,
                draw_n=eff.amount, keep_n=keep_n,
                prompt=f"[{p.name}] keep 1 bonus card from {bird.name}",
            )
            self._log(f"  {bird.name}: drew {eff.amount} bonus, kept {keep_n}")
        elif eff.kind == EffectKind.DRAW_TRAY_ALL:
            taken = list(st.tray)
            st.tray.clear()
            p.hand.extend(taken)
            st.refill_tray()
            self._log(f"  {bird.name}: drew {len(taken)} tray cards: {[b.name for b in taken]}")
        elif eff.kind == EffectKind.PINK_OPP_PREDATOR_DIE:
            avail = [(f, c) for f, c in st.birdfeeder.counts.items() if c > 0]
            if not avail:
                if st.birdfeeder.total() > 0:
                    st.birdfeeder.reroll(st.rng)
                    avail = [(f, c) for f, c in st.birdfeeder.counts.items() if c > 0]
                if not avail:
                    self._log(f"  {bird.name} (pink): birdfeeder empty, no die gained")
                    return
            ch = self._ask(agent, Decision(
                type=DecisionType.GAIN_FOOD_PICK_DIE,
                player_id=p.id,
                prompt=f"[{p.name}] {bird.name} pink: gain 1 die from birdfeeder",
                choices=[Choice(label=f"{f.value}({c})", payload=f) for f, c in avail],
            ))
            chosen: Food = ch.payload
            st.birdfeeder.counts[chosen] -= 1
            p.food[chosen] += 1
            self._log(f"  {bird.name} (pink): {p.name} +1 {chosen.value} from birdfeeder")
        elif eff.kind == EffectKind.TRADE_FOOD:
            give_choices = [f for f, n in p.food.items() if n > 0]
            if not give_choices:
                self._log(f"  {bird.name}: no food to trade, power skipped")
                return
            if len(give_choices) == 1:
                give_food = give_choices[0]
            else:
                ch = self._ask(agent, Decision(
                    type=DecisionType.BIRD_POWER_PICK_FOOD,
                    player_id=p.id,
                    prompt=f"[{p.name}] {bird.name}: choose food to trade away",
                    choices=[Choice(label=f.value, payload=f) for f in give_choices],
                ))
                give_food = ch.payload
            receive_choices = [f for f in ALL_FOODS if f != give_food and st.food_supply.get(f, 0) > 0]
            if not receive_choices:
                self._log(f"  {bird.name}: no other food type in supply, trade skipped")
                return
            if len(receive_choices) == 1:
                get_food = receive_choices[0]
            else:
                ch = self._ask(agent, Decision(
                    type=DecisionType.BIRD_POWER_PICK_FOOD,
                    player_id=p.id,
                    prompt=f"[{p.name}] {bird.name}: choose food to receive",
                    choices=[Choice(label=f.value, payload=f) for f in receive_choices],
                ))
                get_food = ch.payload
            p.food[give_food] -= 1
            st.food_supply[give_food] += 1
            st.food_supply[get_food] -= 1
            p.food[get_food] += 1
            self._log(f"  {bird.name}: traded 1 {give_food.value} for 1 {get_food.value}")
        elif eff.kind == EffectKind.EACH_PLAYER_GAIN_BIRDFEEDER_DIE:
            ch = self._ask(agent, Decision(
                type=DecisionType.BIRD_POWER_PICK_BIRD,
                player_id=p.id,
                prompt=f"[{p.name}] {bird.name}: who gains a birdfeeder die first?",
                choices=[Choice(label=q.name, payload=q.id) for q in st.players],
            ))
            first_id = ch.payload
            ordered = sorted(st.players, key=lambda q: q.id != first_id)
            for q in ordered:
                if not self._gain_one_birdfeeder_die(agent, q, bird.name):
                    break
        elif eff.kind == EffectKind.DISCARD_EGG_GAIN_WILD:
            has_egg = any(pb2.eggs > 0 for row in p.board.values() for pb2 in row)
            if not has_egg:
                self._log(f"  {bird.name}: no eggs to discard, power skipped")
                return
            self._discard_an_egg(agent, p, f"{bird.name} power")
            for _ in range(eff.amount):
                avail = [f for f, n in st.food_supply.items() if n > 0]
                if not avail:
                    break
                ch = self._ask(agent, Decision(
                    type=DecisionType.BIRD_POWER_PICK_FOOD,
                    player_id=p.id,
                    prompt=f"[{p.name}] gain 1 wild food from supply ({bird.name})",
                    choices=[Choice(label=f.value, payload=f) for f in avail],
                ))
                food: Food = ch.payload
                st.food_supply[food] -= 1
                p.food[food] += 1
                self._log(f"  {bird.name}: gained 1 {food.value} from supply")
        elif eff.kind == EffectKind.DISCARD_FOOD_TUCK_FROM_DECK:
            if eff.food and p.food.get(eff.food, 0) >= 1:
                p.food[eff.food] -= 1
                st.food_supply[eff.food] += 1
                actual = 0
                for _ in range(eff.amount):
                    card = st.draw_bird()
                    if card:
                        pb.tucked_cards += 1
                        actual += 1
                        self._log(f"  {bird.name}: tucked {card.name} from deck")
                    else:
                        break
                self._log(f"  {bird.name}: discarded 1 {eff.food.value}, tucked {actual} from deck")
            else:
                self._log(f"  {bird.name}: can't pay {eff.food.value if eff.food else '?'}, power skipped")
        elif eff.kind == EffectKind.PREDATOR_TUCK:
            threshold = eff.extra[0]
            card = st.draw_bird()
            if card:
                if card.wingspan_cm and card.wingspan_cm < threshold:
                    pb.tucked_cards += 1
                    self._log(f"  {bird.name}: tucked {card.name} (wingspan {card.wingspan_cm}cm < {threshold}cm)")
                else:
                    st.bird_discard.append(card)
                    self._log(f"  {bird.name}: discarded {card.name} (wingspan {card.wingspan_cm}cm >= {threshold}cm)")
        elif eff.kind == EffectKind.LAY_EGG_ALL_NEST:
            nest_type = eff.extra[0] if eff.extra else None
            if nest_type is None:
                return
            for row in p.board.values():
                for target_pb in row:
                    if target_pb.bird.nest == nest_type and target_pb.eggs < target_pb.bird.egg_limit:
                        target_pb.eggs += 1
                        self._log(f"  {bird.name}: laid egg on {target_pb.bird.name} ({nest_type.value} nest)")
        elif eff.kind == EffectKind.ALL_PLAYERS_LAY_ON_NEST:
            nest_type = eff.extra[0] if eff.extra else None
            if nest_type is None:
                return
            for q in st.players:
                self._lay_one_egg_on_nest(agent, q, nest_type, optional=False)
            self._lay_one_egg_on_nest(agent, p, nest_type, optional=True)
            self._log(f"  {bird.name}: all players lay on {nest_type.value} birds")
        elif eff.kind == EffectKind.DRAFT_CARDS:
            n_draw = len(st.players) + 1
            pool: list[Bird] = []
            for _ in range(n_draw):
                ...
                if card:
                    pool.append(card)
            if not pool:
                self._log(f"  {bird.name}: no cards to draft")
                return
            # Active player picks first, then each other player in order
            draft_order = [p] + [q for q in st.players if q.id != p.id]
            for q in draft_order:
                if not pool:
                    break
                if len(pool) == 1:
                    q.hand.append(pool.pop(0))
                    self._log(f"  {bird.name}: {q.name} auto-keeps last card {q.hand[-1].name}")
                    break
                ch = self._ask(agent, Decision(
                    type=DecisionType.BIRD_POWER_PICK_CARD,
                    player_id=q.id,
                    prompt=f"[{q.name}] draft 1 card from {bird.name} pool",
                    choices=[Choice(label=c.name, payload=c) for c in pool],
                ))
                chosen: Bird = ch.payload
                pool.remove(chosen)
                q.hand.append(chosen)
                self._log(f"  {bird.name}: {q.name} drafted {chosen.name}")
            # Active player keeps any remaining cards (the +1 extra)
            if pool:
                p.hand.extend(pool)
                self._log(f"  {bird.name}: {p.name} keeps {len(pool)} extra card(s)")
        elif eff.kind == EffectKind.REPEAT_PREDATOR:
            candidates = [
                (i, target_pb)
                for i, target_pb in enumerate(p.board[habitat])
                if target_pb is not pb and target_pb.bird.predator
            ]
            if not candidates:
                self._log(f"  {bird.name}: no predator in {habitat.value}, power skipped")
                return
            if len(candidates) == 1:
                _, target_pb = candidates[0]
            else:
                ch = self._ask(agent, Decision(
                    type=DecisionType.BIRD_POWER_PICK_BIRD,
                    player_id=p.id,
                    prompt=f"[{p.name}] {bird.name}: repeat which predator's power?",
                    choices=[Choice(label=target_pb.bird.name, payload=i) for i, target_pb in candidates],
                ))
                idx = ch.payload
                target_pb = p.board[habitat][idx]
            self._log(f"  {bird.name}: repeating predator {target_pb.bird.name}'s power")
            self._dispatch_power(agent, p, target_pb, habitat, "repeat")
