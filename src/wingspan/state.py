...
# All factory functions are collected here, above the models that use them.
# Factories may reference classes defined later in this file — their bodies
# execute only at instance-creation time, after the module has fully loaded.
...
def _new_birdfeeder() -> Birdfeeder:
    return Birdfeeder()


def _new_food_pool() -> FoodPool:
    return FoodPool()
...
# ---------------------------------------------------------------------------
# Main character: GameState
# Leaf models (Player, Board, PlayedBird, Birdfeeder, FoodPool, RoundGoalResult)
# are defined in the next section. Forward references in GameState's annotations
...
class GameState(pydantic.BaseModel):
    ...
    birdfeeder: Birdfeeder = pydantic.Field(default_factory=_new_birdfeeder)
...
class Player(pydantic.BaseModel):
    ...
    food: FoodPool = pydantic.Field(default_factory=_new_food_pool)
    board: Board = pydantic.Field(default_factory=_new_board)
...
class PlayedBird(pydantic.BaseModel):
    ...
    cached_food: FoodPool = pydantic.Field(default_factory=_new_food_pool)
    tucked_cards: int = 0
...
class Birdfeeder(pydantic.BaseModel):
    ...
    counts: FoodPool = pydantic.Field(default_factory=_new_food_pool)
...
# Rebuild in dependency order: leaf types before composite types that reference them.

FoodPool.model_rebuild()
RoundGoalResult.model_rebuild()
PlayedBird.model_rebuild()
Birdfeeder.model_rebuild()
Board.model_rebuild()
