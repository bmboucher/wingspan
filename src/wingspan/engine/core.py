...
from wingspan.engine import actions, ledger, log_format, scoring, setup_flow
...
class Engine:
    ...
    def _setup_phase_fixed(
        ...
        for player in self.state.players:
            ...
            setup_flow.apply_setup_choice(
                self, player, dealt_cards, dealt_bonus, sc, defer_food=defer_food
            )
            setup_flow.resolve_deferred_setup_bonus(self, player, dealt_bonus, sc)
            setup_flow.resolve_deferred_setup_food(
                self,
                player,
    ...
    def _resolve_setup_choice(
        ...
        *,
        ...
        setup_flow.apply_setup_choice(
            self, player, dealt_cards, dealt_bonus, chosen, defer_food=defer_food
        )
        ...
        setup_flow.resolve_deferred_setup_bonus(self, player, dealt_bonus, chosen)
        setup_flow.resolve_deferred_setup_food(
            self,
            player,
            agents[player.id],
            len(chosen.kept_cards),
            defer_food=defer_food,
        )
        self.events.end_event()
