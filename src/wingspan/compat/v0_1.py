...
class PolicyValueNetV01(core.PolicyValueNet):
    ...
    def _choice_embed_offsets(self) -> core.ChoiceEmbedOffsets:
        ...
        The v0.1 choice row predates the ``becomes_playable`` stripe added in v0.6
        and uses v0.8 board geometry (pre-0.9). Returns frozen v0.8 offsets so
        ``_embed_choices`` slices at the correct columns (``PolicyValueNetV00``
        inherits this)."""
        from wingspan.compat import v0_8  # local: avoids import cycle
    ...
    def _embed_choices(
        ...
        v0.1 rows carry ``board_idx`` (15 embedded board slots) at the v0.8
