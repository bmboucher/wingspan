"""Pre-1.7 artifact compat shim: freeze the static (egg-blind) bonus potential
pricing, on both nets.

**What changed in v1.7 (encoding behavior — no shape change).** The bonus
...
pricing in place after live encoding, via the ``refill_*_static`` seams next
to the live fills.
...
(re-chained from the live classes), so every era <= 1.5 freezes the static
bonus potentials too, composed with its own strips and refills via
``super()`` chaining.
...
class PolicyValueNetV1_6(core.PolicyValueNet):
    """``PolicyValueNet`` with the pre-1.7 static bonus potential pricing:
    every bonus-carrying choice row prices ``hand_potential`` /
    ``tray_potential`` by the static ``bonus_categories`` tag alone, so the
    egg-counting dynamic cards read 0. Geometry is live — v1.7 is
    behavior-only, so this class overrides ``encode_choices`` and nothing
    else."""
    ...
    def encode_choices(
        ...
        """Encode at live geometry, then overwrite each bonus-carrying row's
        potential scalars with the static pre-1.7 pricing.

        The refill runs at full live width — this is the deepest override
        above ``core`` — and targets ``layout._OFF_BONUS_VALUE``, an offset
        before ``becomes_playable`` and therefore before every column any
        subclass strips (``v1_5``'s ``goal_delta_ignoring_eggs`` tail,
        ``v1_3``'s ``resets_feeder``, ``v1_0``'s ``becomes_unplayable``), so
        the whole chain composes with no offset math."""
